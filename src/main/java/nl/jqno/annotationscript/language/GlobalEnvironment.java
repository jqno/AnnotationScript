...
public final class GlobalEnvironment {
    private static final List<Tuple2<String, Fn>> GLOBAL = List.of(
        ...
        Tuple.of("-", 
            new Fn("-", Fn.VARARG, params -> new AstFloat(params.tail().foldLeft(params.head().asFloat(), (acc, curr) -> acc - curr.asFloat())))),
        Tuple.of("*", 
            new Fn("*", Fn.VARARG, params -> new AstFloat(params.foldLeft(1.0, (acc, curr) -> acc * curr.asFloat())))),
        Tuple.of("/", 
            new Fn("/", Fn.VARARG, params -> new AstFloat(params.tail().foldLeft(params.head().asFloat(), (acc, curr) -> acc / curr.asFloat())))),
        Tuple.of("begin", 
            new Fn("begin", Fn.VARARG, params -> params.last())),
        Tuple.of("pi", 
            new Fn("pi", 0, params -> new AstFloat(Math.PI)))
    );
